...
$sql = "SELECT * FROM books WHERE id=".mysql_real_escape_string($book_id);
if (!($res = $mysqli->query($sql)))
    die("Error: " . $mysqli->error);

$brow = $res->fetch_array();

echo "<h1>" . htmlspecialchars($brow['title']) . "</h1>";
...
for ($i = 0; $i < $cres->num_rows; $i++)
{
    ...
    echo htmlspecialchars($crow['name']);
...
// Edition
echo "Edition: " . htmlspecialchars($brow['edition']) . "</br>";
...
if(isset($languageCodes[$brow['language']])){
	echo $languageCodes[$brow['language']];
}else{
	echo htmlspecialchars($brow['language']);
}

echo "</br>";

// Insert date
echo "Insert Date: " . date('j M Y G:i:s', strtotime($brow['inserted'])) . "</br>";
